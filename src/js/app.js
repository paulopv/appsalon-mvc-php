...
function iniciarApp() {
    ...
    tabs(); // Cambia las secciones al dar click en los botones
...
function mostrarSeccion() {
    ...
    //Seleccionar la seccion actual
    const pasoSelector = `#paso-${paso}`;
    const seccion = document.querySelector(pasoSelector);
    seccion.classList.add('mostrar');

    //Quitar la clase actual de los botones
    const tabAnterior = document.querySelector('.actual');
    if (tabAnterior) {
        tabAnterior.classList.remove('actual');
    }

    //Resaltar el boton actual
...
function seleccionarFecha() {
    ...
    inputFecha.addEventListener('input', function(e) {
        ...
        if ([6, 0].includes(dia)) {
            e.target.value = '';
            mostrarAlerta('No abrimos los fines de semana', 'error', '.formulario');
        } else {
...
function mostrarAlerta(mensaje, tipo, elemento, desaparece = true) {
    //Previene que se muestren multiples alertas
